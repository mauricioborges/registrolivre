describe("Controller: NewCompanyController", function() {
 var $rootScope, $scope, $controller, $httpBackend, document, messages;
    ...
    it("VerifyDate should return true when date is valid", function(){
        var controller = $controller('NewCompanyController',{$scope: $scope });
        var date = $scope.verifyDate("10/10/2010");
        expect(date).to.be.true;

    });

    it("VerifyDate should return false when date is invalid", function(){
        var controller = $controller('NewCompanyController',{$scope: $scope });
        var date = $scope.verifyDate("32/13/9999");
        expect(date).to.be.false;
    });

    it("VerifyDate should return true when date is empty", function(){
        var controller = $controller('NewCompanyController',{$scope: $scope });
        var date = $scope.verifyDate("");
        expect(date).to.be.true;

    });

    it("Should detect when opening date is valid", function(){
        var controller = $controller('NewCompanyController',{$scope: $scope });
        $scope.verifyOpenDate("10/10/2010");
        expect($scope.openDateValidation).to.be.equal($scope.VALIDATION.VALID);
    });

    it("Should detect when opening date is invalid", function(){
        var controller = $controller('NewCompanyController',{$scope: $scope });
        $scope.verifyOpenDate("32/13/9999");
        expect($scope.openDateValidation).to.be.equal($scope.VALIDATION.INVALID);
    });

    it("Should detect when opening date is empty", function(){
        var controller = $controller('NewCompanyController',{$scope: $scope });
        $scope.verifyOpenDate("");
        expect($scope.openDateValidation).to.be.equal($scope.VALIDATION.VALID);
    });

    it("Should detect when issue date is valid", function(){
        var controller = $controller('NewCompanyController',{$scope: $scope });
        $scope.verifyEmissionDate("10/10/2010");
        expect($scope.emissionDateValidation).to.be.equal($scope.VALIDATION.VALID);
    });

    it("Should detect when issue date is invalid", function(){
        var controller = $controller('NewCompanyController',{$scope: $scope });
        $scope.verifyEmissionDate("40/99/9999");
        expect($scope.emissionDateValidation).to.be.equal($scope.VALIDATION.INVALID);
    });
